...
//		}
		...
		public Client update(@RequestBody Client c) {
			if(cr.findById(c.getId()).isEmpty())
				throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Le client n'existe pas");
			c.setCreatedAt(cr.findById(c.getId()).get().getCreatedAt());
			c.setUtilisateur(cr.findById(c.getId()).get().getUtilisateur());
			c.setCommandes(cr.findById(c.getId()).get().getCommandes());
			return cr.save(c);
		}
		...
		@Transactional// On sait jamais ?
		public Utilisateur create(UtilisateurClient uc) {
			Client c = cr.save(new Client(uc));
