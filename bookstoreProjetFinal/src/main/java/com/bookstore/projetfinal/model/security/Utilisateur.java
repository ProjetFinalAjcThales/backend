...
public class Utilisateur {
	...
	public Utilisateur(UtilisateurClient uc) {
		this(uc.getMail(), uc.getMdp());
	}
	...
	/*
	 * public UserDetails toUserDetails() { List<GrantedAuthority> auths =
	 * roles.stream().map(Role::name)// transformation en String (role ->
	 * role.name()) .map(SimpleGrantedAuthority::new)// transformation en objet
	 * (name -> new SimpleGrantedAuthority(name)) .collect(Collectors.toList());
